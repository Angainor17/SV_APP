...
Добавление нового функционального блока - квизы (тесты, опросники) с вопросами по темам. Квизы
привязаны к существующей MediaWiki инсталляции svremya.su.
...
- Пропуск вопроса: пользователь может перейти к следующему вопросу без ответа, вернуться позже. Но
  квиз считается завершённым только когда все вопросы отвечены.
- Для завершения квиза на последнем вопросе показывать: "Осталось N вопросов без ответа" если есть
  пропущенные

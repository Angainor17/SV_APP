...
class OpenRootAction(activity: Activity) : Action(activity, ActionCode.OPEN_ROOT, "openRoot", -1) {
    ...
    override fun isVisible(tree: NetworkTree): Boolean {
        ...
        var t: NetworkTree? = tree
        while (t != null) {
            if (t is RootTree) {
                return t == myLibrary.getRootTree()
            }
            t = t.parent as? NetworkTree
        }
        return false
    }

    override fun run(tree: NetworkTree) {
        val activity = myActivity as NetworkLibraryActivity

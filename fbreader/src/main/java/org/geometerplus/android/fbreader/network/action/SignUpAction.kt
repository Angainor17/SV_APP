...
class SignUpAction(activity: Activity) : Action(activity, ActionCode.SIGNUP, "signUp", -1) {
    ...
    override fun isVisible(tree: NetworkTree): Boolean {
        ...
        val mgr = tree.getLink()?.authenticationManager()
    ...
    override fun run(tree: NetworkTree) {
        ...
        if (link != null) {
            try {
                myActivity.startActivity(Util.authorisationIntent(link, myActivity, UserRegistrationActivity::class.java))

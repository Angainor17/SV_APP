...
class AuthorListTree internal constructor(root: RootTree) : FirstLevelTree(root, ROOT_BY_AUTHOR) {
        ...
        return when (event) {
            BookEvent.Added, BookEvent.Updated -> {
                ...
                if (bookAuthors.isEmpty()) {
                    ...
                    for (a in bookAuthors) {
                        changed = createAuthorSubtree(a) || changed
                    }
                }
                changed
            }
    ...
    private fun createAuthorSubtree(author: Author): Boolean {
        val temp = AuthorTree(collection, pluginCollection, author)
        val position = Collections.binarySearch(subtrees(), temp)
        return if (position >= 0) {
            false
        } else {
            AuthorTree(this, author, -position - 1)
            true
        }
    }
}

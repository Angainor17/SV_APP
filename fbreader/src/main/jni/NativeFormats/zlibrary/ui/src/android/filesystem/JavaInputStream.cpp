...
JavaInputStream::JavaInputStream(const std::string &name,
                                 shared_ptr<FileEncryptionInfo> encryptionInfo) : myName(name),
                                                                                  myEncryptionInfo(
                                                                                          encryptionInfo),
                                                                                  myNeedRepositionToStart(
                                                                                          false),
                                                                                  myMarkSupported(
                                                                                          false) {
    myJavaFile = 0;
...
std::size_t JavaInputStream::readToBuffer(JNIEnv *env, char *buffer, std::size_t maxSize) {
    ...
    jint result =
            AndroidUtil::Method_java_io_InputStream_read->call(myJavaInputStream, myJavaBuffer,
                                                               (jint) 0, (jint) maxSize);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return 0;
...
std::size_t JavaInputStream::skip(JNIEnv *env, std::size_t offset) {
    std::size_t result =
            (std::size_t) AndroidUtil::Method_java_io_InputStream_skip->call(myJavaInputStream,
                                                                             (jlong) offset);
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        return 0;

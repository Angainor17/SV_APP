...
shared_ptr<ZLZipEntryCache>
ZLZipEntryCache::cache(const std::string &containerName, ZLInputStream &containerStream) {
...
ZLZipEntryCache::ZLZipEntryCache(const std::string &containerName, ZLInputStream &containerStream)
    ...
    while (header.readFrom(containerStream)) {
        ...
        if (header.Signature == (unsigned long) ZLZipHeader::SignatureLocalFile) {
            ...
            if ((unsigned int) containerStream.read((char *) entryName.data(), header.NameLength) ==
                header.NameLength) {
...
void ZLZipEntryCache::collectFileNames(std::vector<std::string> &names) const {
    for (std::map<std::string, Info>::const_iterator it = myInfoMap.begin();
         it != myInfoMap.end(); ++it) {

...
ZLLanguageDetector::LanguageInfo::LanguageInfo(const std::string &language,
                                               const std::string &encoding) : Language(language),
                                                                              Encoding(encoding) {
}
    ...
    if (!dir.isNull()) {
        ...
        for (std::vector<std::string>::const_iterator it = fileNames.begin();
            ...
            if (index != -1) {
                ...
                shared_ptr<ZLStatisticsBasedMatcher> matcher = new ZLStatisticsBasedMatcher(
...
ZLLanguageDetector::findInfo(const char *buffer, std::size_t length, int matchingCriterion) {
    std::string naive;
...
shared_ptr<ZLLanguageDetector::LanguageInfo>
ZLLanguageDetector::findInfoForEncoding(const std::string &encoding, const char *buffer,
                                        std::size_t length, int matchingCriterion) {

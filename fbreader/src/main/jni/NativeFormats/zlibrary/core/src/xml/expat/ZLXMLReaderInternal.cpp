...
void ZLXMLReaderInternal::fStartElementHandler(void *userData, const char *name,
...
static void
parseExtraDTDEntities(XML_Parser parser, const std::map<std::string, std::string> &entityMap) {
    XML_Parser entityParser = XML_ExternalEntityParserCreate(parser, 0, 0);
    std::string buffer;
...
ZLXMLReaderInternal::ZLXMLReaderInternal(ZLXMLReader &reader, const char *encoding) : myReader(
        reader) {

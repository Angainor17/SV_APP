...
ZLStatistics::ZLStatistics(std::size_t charSequenceSize, std::size_t volume,
...
void ZLMapBasedStatistics::scaleToShort() {
    const std::size_t maxFrequency = std::max_element(myDictionary.begin(), myDictionary.end(),
                                                      LessFrequency())->second;
...
ZLArrayBasedStatistics::ZLArrayBasedStatistics() : ZLStatistics(),
                                                   myCapacity(0), myBack(0), mySequences(0),
                                                   myFrequencies(0) {
}

ZLArrayBasedStatistics::ZLArrayBasedStatistics(std::size_t charSequenceSize, std::size_t size,
                                               std::size_t volume, unsigned long long squaresVolume)
        :
...
shared_ptr<ZLStatisticsItem> ZLArrayBasedStatistics::end() const {
    return new ZLArrayBasedStatisticsItem(myCharSequenceSize,
                                          mySequences + myBack * myCharSequenceSize,
                                          myFrequencies + myBack, myBack);
}

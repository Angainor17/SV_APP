...
ZLFile::ZLFile(const std::string &path, const std::string &mimeType) : myPath(path),
                                                                       myMimeType(mimeType),
                                                                       myMimeTypeIsUpToDate(
                                                                               !mimeType.empty()),
    ...
    if (it != forcedFiles.end()) {
        ...
        if (ZLStringUtil::stringEndsWith(lowerCaseName, ".gz")) {
            myNameWithoutExtension = myNameWithoutExtension.substr(0,
                                                                   myNameWithoutExtension.length() -
                                                                   3);

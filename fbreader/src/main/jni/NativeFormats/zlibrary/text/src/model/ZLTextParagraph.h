...
inline ZLTextControlEntry::ZLTextControlEntry(ZLTextKind kind, bool isStart) : myKind(kind),
                                                                               myStart(isStart) {}
...
inline ZLHyperlinkType
ZLTextHyperlinkControlEntry::hyperlinkType() const { return myHyperlinkType; }
...
inline void ZLTextParagraph::addEntry(char *address) {
    if (myEntryNumber == 0)
        myFirstEntryAddress = address;
    ++myEntryNumber;
}

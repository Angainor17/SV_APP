...
public:
    struct Label {
        Label(shared_ptr<ZLTextModel> model, int paragraphNumber) : Model(model), ParagraphNumber(
                paragraphNumber) {}
...
public:
    ...
    public:
        virtual Label
        match(const std::map<std::string, Label> &lMap, const std::string &id) const = 0;
    };
...
inline const std::map<std::string, shared_ptr<ZLTextModel> > &
BookModel::footnotes() const { return myFootnotes; }

inline const std::map<std::string, BookModel::Label> &
BookModel::internalHyperlinks() const { return myInternalHyperlinks; }
...
inline const std::vector<shared_ptr<ContentsTree> > &
ContentsTree::children() const { return myChildren; }

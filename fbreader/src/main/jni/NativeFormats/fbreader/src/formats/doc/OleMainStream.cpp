...
OleMainStream::OleMainStream(shared_ptr<OleStorage> storage, OleEntry oleEntry,
                             shared_ptr<ZLInputStream> stream) : OleStream(storage, oleEntry,
                                                                           stream) {
}
...
bool OleMainStream::open(bool doReadFormattingData) {
    ...
    if (!result) {
        ...
        ZLLogger::Instance().println("DocPlugin",
                                     "cant't find table stream, building own simple piece table, that includes all charachters");
...
bool OleMainStream::readFIB(const char *headerBuffer) {
    ...
    if (flags & 0x1000) { //flag for using extending charset
        ZLLogger::Instance().println("DocPlugin",
                                     "File uses extended character set (get_word8_char)");
    } else {
...
void
OleMainStream::splitPieces(const Pieces &s, Pieces &dest1, Pieces &dest2, Piece::PieceType type1,
                           Piece::PieceType type2, int boundary) {
    Pieces source = s;
    dest1.clear();
    dest2.clear();
...
std::string OleMainStream::getPiecesTableBuffer(const char *headerBuffer, OleStream &tableStream) {
    ...
    unsigned int clxLength = OleUtil::getU4Bytes(headerBuffer,
                                                 0x01A6); //offset for value of CLX structure length

    //1 step : loading CLX table from table stream
    char *clxBuffer = new char[clxLength];
    if (!tableStream.seek(clxOffset, true)) {
        ZLLogger::Instance().println("DocPlugin",
                                     "getPiecesTableBuffer -- error for seeking to CLX structure");
        return std::string();
    }
    if (tableStream.read(clxBuffer, clxLength) != clxLength) {
        ZLLogger::Instance().println("DocPlugin",
                                     "getPiecesTableBuffer -- CLX structure length is invalid");
        return std::string();
    }
    ...
    while ((i = clx.find_first_of(0x02, from)) != std::string::npos) {
        if (clx.size() < i + 1 + 4) {
            ZLLogger::Instance().println("DocPlugin",
                                         "getPiecesTableBuffer -- CLX structure has invalid format");
            return std::string();
        }
...
bool OleMainStream::readPieceTable(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    for (j = 0;; j += 4) {
        if (piecesTableBuffer.size() < j + 4) {
            ZLLogger::Instance().println("DocPlugin",
                                         "invalid piece table, cp ends not with a lastcp");
            break;
        }
    ...
    for (std::size_t k = 0; k < cp.size() - 1; ++k) {
        ...
        if (piecesTableBuffer.size() < substrFrom + 8) {
            ZLLogger::Instance().println("DocPlugin",
                                         "invalid piece table, problems with descriptors reading");
            break;
        }
    ...
    for (std::size_t i = 0; i < minValidSize; ++i) {
        ...
        int fcValue = OleUtil::get4Bytes(descriptors.at(i).c_str(),
                                         0x2); //offset for piece structure
        Piece piece;
    ...
    splitPieces(myPieces, piecesText, piecesFootnote, Piece::PIECE_TEXT, Piece::PIECE_FOOTNOTE,
                ccpText);
    splitPieces(piecesFootnote, piecesFootnote, piecesOther, Piece::PIECE_FOOTNOTE,
                Piece::PIECE_OTHER, ccpFtn);
...
bool OleMainStream::readBookmarks(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginNamesInfo = OleUtil::getU4Bytes(headerBuffer,
                                                      0x142); // address of SttbfBkmk structure
    std::size_t namesInfoLength = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                    0x146); // length of SttbfBkmk structure
    ...
    for (unsigned int i = 0; i < recordsNumber; ++i) {
        ...
        unsigned int length =
                OleUtil::getU2Bytes(buffer.c_str(), offset) * 2; //length of string in bytes
    ...
    unsigned int beginCharPosInfo = OleUtil::getU4Bytes(headerBuffer,
                                                        0x14A); // address of plcfBkmkf structure
    std::size_t charPosInfoLen = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                   0x14E); // length of plcfBkmkf structure
...
bool OleMainStream::readStylesheet(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginStshInfo = OleUtil::getU4Bytes(headerBuffer,
                                                     0xa2); // address of STSH structure
    std::size_t stshInfoLength = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                   0xa6); // length of STSH structure
    ...
    if (tableStream.read(buffer, stshInfoLength) != stshInfoLength) {
        ZLLogger::Instance().println("DocPlugin",
                                     "problems with reading STSH structure, invalid length");
        return false;
    }
    ...
    do { //make it in while loop, because some base style can be after their successors
        ...
        for (std::size_t index = 0, offset = 2 + (std::size_t) OleUtil::getU2Bytes(buffer, 0);
             index < stdCount; index++, offset += 2 + stdLen) {
            ...
            nameLen = nameLen * 2 +
                      2; //from Unicode characters to bytes + Unicode null charachter length
            pos += 2 + nameLen;
            if (pos % 2 != 0) {
                ++pos;
            }
...
bool OleMainStream::readCharInfoTable(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginCharInfo = OleUtil::getU4Bytes(headerBuffer,
                                                     0xfa); // address of PlcfbteChpx structure
    std::size_t charInfoLength = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                   0xfe); // length of PlcfbteChpx structure
    if (charInfoLength < 4) {
        return false;
    }
    ...
    for (std::size_t index = 0, offset = (size + 1) * 4;
         index < size; ++index, offset += CHPX_SIZE) {
    ...
    for (std::size_t index = 0; index < charBlocks.size(); ++index) {
        ...
        unsigned int crun = OleUtil::getU1Byte(formatPageBuffer,
                                               0x1ff); //offset with crun (count of 'run of text')
        for (unsigned int index2 = 0; index2 < crun; ++index2) {
            unsigned int offset = OleUtil::getU4Bytes(formatPageBuffer, index2 * 4);
            unsigned int chpxOffset =
                    2 * OleUtil::getU1Byte(formatPageBuffer, (crun + 1) * 4 + index2);
...
bool OleMainStream::readFloatingImages(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginPicturesInfo = OleUtil::getU4Bytes(headerBuffer,
                                                         0x01DA); // address of Plcspa structure
    if (beginPicturesInfo == 0) {
        return true; //there's no office art objects
    }
    unsigned int picturesInfoLength = OleUtil::getU4Bytes(headerBuffer,
                                                          0x01DE); // length of Plcspa structure
    ...
    for (std::size_t index = 0, tOffset = (size + 1) * 4;
         index < size; ++index, tOffset += SPA_SIZE) {
    ...
    unsigned int beginOfficeArtContent = OleUtil::getU4Bytes(headerBuffer,
                                                             0x22A); // address of DggInfo structure
    if (beginOfficeArtContent == 0) {
        return true; //there's no office art objects
    }
    unsigned int officeArtContentLength = OleUtil::getU4Bytes(headerBuffer,
                                                              0x022E); // length of DggInfo structure
    ...
    if (newTableStream->open() && newMainStream->open()) {
        myFLoatImageReader = new DocFloatImageReader(beginOfficeArtContent, officeArtContentLength,
                                                     newTableStream, newMainStream);
...
bool OleMainStream::readParagraphStyleTable(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginParagraphInfo = OleUtil::getU4Bytes(headerBuffer,
                                                          0x102); // address of PlcBtePapx structure
    std::size_t paragraphInfoLength = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                        0x106); // length of PlcBtePapx structure
    ...
    for (std::size_t index = 0, tOffset = (size + 1) * 4;
         index < size; ++index, tOffset += PAPX_SIZE) {
    ...
    for (std::size_t index = 0; index < paragraphBlocks.size(); ++index) {
        ...
        const unsigned int paragraphsCount = OleUtil::getU1Byte(formatPageBuffer,
                                                                0x1ff); //offset with 'cpara' value (count of paragraphs)
        for (unsigned int index2 = 0; index2 < paragraphsCount; ++index2) {
            ...
            unsigned int papxOffset =
                    OleUtil::getU1Byte(formatPageBuffer, (paragraphsCount + 1) * 4 + index2 * 13) *
                    2;
            if (papxOffset <= 0) {
                continue;
            }
...
bool OleMainStream::readSectionsInfoTable(const char *headerBuffer, const OleEntry &tableEntry) {
    ...
    unsigned int beginOfText = OleUtil::getU4Bytes(headerBuffer,
                                                   0x18); //address of text's begin in main stream
    unsigned int beginSectInfo = OleUtil::getU4Bytes(headerBuffer,
                                                     0xca); //address if PlcfSed structure

    std::size_t sectInfoLen = (std::size_t) OleUtil::getU4Bytes(headerBuffer,
                                                                0xce); //length of PlcfSed structure
    if (sectInfoLen < 4) {
        return false;
    }
    ...
    for (std::size_t index = 0, tOffset = (decriptorsCount + 1) * 4;
         index < decriptorsCount; ++index, tOffset += SED_SIZE) {
    ...
    for (std::size_t index = 0; index < sectPage.size(); ++index) {
        if (sectPage.at(index) ==
            0xffffffffUL) { //check for invalid record, to make default section info
...
void
OleMainStream::getStyleInfo(unsigned int papxOffset, const char *grpprlBuffer, unsigned int bytes,
                            Style &styleInfo) {
    ...
    while (bytes >= offset + 2) {
        ...
        switch (OleUtil::getU2Bytes(grpprlBuffer, papxOffset + offset)) {
            case 0x2403:
                styleInfo.Alignment = (Style::AlignmentType) OleUtil::getU1Byte(grpprlBuffer,
                                                                                papxOffset +
                                                                                offset + 2);
                break;
            case 0x4610:
            ...
            case 0xc615: // ChgTabs
                ...
                break;
            case 0x840e:
                styleInfo.RightIndent = (int) OleUtil::getU2Bytes(grpprlBuffer,
                                                                  papxOffset + offset + 2);
                break;
            case 0x840f:
                styleInfo.LeftIndent = (int) OleUtil::getU2Bytes(grpprlBuffer,
                                                                 papxOffset + offset + 2);
                break;
            case 0x8411:
                styleInfo.FirstLineIndent = (int) OleUtil::getU2Bytes(grpprlBuffer,
                                                                      papxOffset + offset + 2);
                break;
            case 0xa413:
                styleInfo.BeforeParagraphIndent = OleUtil::getU2Bytes(grpprlBuffer,
                                                                      papxOffset + offset + 2);
                break;
            case 0xa414:
                styleInfo.AfterParagraphIndent = OleUtil::getU2Bytes(grpprlBuffer,
                                                                     papxOffset + offset + 2);
                break;
            case 0x2407:
                styleInfo.HasPageBreakBefore =
                        OleUtil::getU1Byte(grpprlBuffer, papxOffset + offset + 2) == 0x01;
                break;
            default:
                break;
        }
...
void OleMainStream::getCharInfo(unsigned int chpxOffset, unsigned int /*styleId*/,
                                const char *grpprlBuffer, unsigned int bytes, CharInfo &charInfo) {
    unsigned int offset = 0;
    while (bytes >= offset + 2) {
...
void OleMainStream::getSectionInfo(const char *grpprlBuffer, std::size_t bytes,
                                   SectionInfo &sectionInfo) {
    unsigned int tmp;
    std::size_t offset = 0;
    while (bytes >= offset + 2) {
...
bool OleMainStream::getInlineImageInfo(unsigned int chpxOffset, const char *grpprlBuffer,
                                       unsigned int bytes, InlineImageInfo &pictureInfo) {
...
//				break;
            case 0x6a03: // location p.105 [MS-DOC]
                pictureInfo.DataPosition = OleUtil::getU4Bytes(grpprlBuffer,
                                                               chpxOffset + offset + 2);
                isFound = true;
                break;
            default:
...
OleMainStream::Style
OleMainStream::getStyleFromStylesheet(unsigned int styleId, const StyleSheet &stylesheet) {
    //TODO optimize it: StyleSheet can be map structure with styleId key
    Style style;
    if (styleId != Style::STYLE_INVALID && styleId != Style::STYLE_NIL &&
        styleId != Style::STYLE_USER) {
...
int OleMainStream::getStyleIndex(unsigned int styleId, const std::vector<bool> &isFilled,
...
unsigned int
OleMainStream::getStyleIdByCharPos(unsigned int charPos, const StyleInfoList &styleInfoList) {
...
bool
OleMainStream::offsetToCharPos(unsigned int offset, unsigned int &charPos, const Pieces &pieces) {
    if (pieces.empty()) {
        return false;
    }
...
bool OleMainStream::readToBuffer(std::string &result, unsigned int offset, std::size_t length,
                                 OleStream &stream) {

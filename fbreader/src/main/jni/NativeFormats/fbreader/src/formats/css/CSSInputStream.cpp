...
CSSInputStream::CSSInputStream(shared_ptr<ZLInputStream> base) : myBaseStream(base), myBuffer(8192),
                                                                 myBufferNoComments(8192) {
...
std::size_t CSSInputStream::read(char *buffer, std::size_t maxSize) {
    ...
    while (ready < maxSize) {
        ...
        if (buffer != 0) {
            std::memcpy(buffer + ready, myBufferNoComments.Content + myBufferNoComments.Offset,
                        len);
        }

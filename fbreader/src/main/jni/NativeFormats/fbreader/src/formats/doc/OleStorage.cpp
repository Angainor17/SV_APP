...
bool OleStorage::init(shared_ptr<ZLInputStream> stream, std::size_t streamSize) {
    ...
    myShortSectorSize =
            1 << OleUtil::getU2Bytes(oleBuf, 0x20); //offset for value of small sector size

    if (readDIFAT(oleBuf) && readBBD(oleBuf) && readSBD(oleBuf) && readProperties(oleBuf) &&
        readAllEntries()) {
        return true;
    }
    clear();
...
bool OleStorage::readDIFAT(char *oleBuf) {
    ...
    for (int i = 0; difatBlock > 0 && i < difatSectorNumbers; ++i) {
        ...
        difatBlock = OleUtil::get4Bytes(buffer, mySectorSize -
                                                4); //next DIFAT block is pointed at the end of the sector
    }
...
bool OleStorage::readProperties(char *oleBuf) {
    int propCur = OleUtil::get4Bytes(oleBuf,
                                     0x30); //offset for address of sector with first property
    if (propCur < 0) {
...
bool OleStorage::readOleEntry(int propNumber, OleEntry &e) {
    ...
    int nameLength = OleUtil::getU2Bytes(property.c_str(),
                                         0x40); //offset for value entry's name length
    e.name.clear();
    ...
    if (chainCur >= 0 &&
        (chainCur <= (int) (myStreamSize / (e.isBigBlock ? mySectorSize : myShortSectorSize)))) {
    ...
    e.length = std::min(e.length,
                        (unsigned int) ((e.isBigBlock ? mySectorSize : myShortSectorSize) *
                                        e.blocks.size()));
    return true;
}

bool OleStorage::countFileOffsetOfBlock(const OleEntry &e, unsigned int blockNumber,
    ...
    if (e.blocks.size() <= (std::size_t) blockNumber) {
        ZLLogger::Instance().println("DocPlugin",
                                     "countFileOffsetOfBlock can't be done, blockNumber is invalid");
        return false;
    }
    if (e.isBigBlock) {
        ...
        if (myEntries.at(myRootEntryIndex).blocks.size() <= (std::size_t) sbdSectorNumber) {
            ZLLogger::Instance().println("DocPlugin",
                                         "countFileOffsetOfBlock can't be done, invalid sbd data");
            return false;
        }
        result = BBD_BLOCK_SIZE +
                 myEntries.at(myRootEntryIndex).blocks.at(sbdSectorNumber) * mySectorSize +
                 sbdSectorMod * myShortSectorSize;
    }
    return true;
}

...
StyleSheetMultiStyleParser::StyleSheetMultiStyleParser(const std::string &pathPrefix,
                                                       shared_ptr<FontMap> fontMap,
                                                       shared_ptr<EncryptionMap> encryptionMap)
        : StyleSheetParser(pathPrefix), myFontMap(fontMap.isNull() ? new FontMap() : fontMap),
...
void StyleSheetTableParser::store(shared_ptr<CSSSelector> selector,
                                  const StyleSheetTable::AttributeMap &map) {
    myStyleTable.addMap(selector, map);
}

StyleSheetParserWithCache::StyleSheetParserWithCache(const ZLFile &file,
...
void StyleSheetParserWithCache::store(shared_ptr<CSSSelector> selector,
                                      const StyleSheetTable::AttributeMap &map) {
...
void StyleSheetParserWithCache::importCSS(const std::string &path) {
    ...
    ZLLogger::Instance().println("CSS-IMPORT",
                                 "Go to process imported file " + fileToImport.path());
    ...
    if (!stream.isNull()) {
        ...
        myEntries.insert(myEntries.end(), importParser.myEntries.begin(),
...
void StyleSheetParserWithCache::applyToTables(StyleSheetTable &table, FontMap &fontMap) const {
    for (std::list<shared_ptr<Entry> >::const_iterator it = myEntries.begin();
         it != myEntries.end(); ++it) {

...
public:
    ...
    static shared_ptr<ZLTextStyleEntry>
    createOrUpdateControl(const AttributeMap &map, shared_ptr<ZLTextStyleEntry> entry = 0);

private:
    void addMap(shared_ptr<CSSSelector> selector, const AttributeMap &map);

    static void
    setLength(ZLTextStyleEntry &entry, ZLTextStyleEntry::Feature featureId, const AttributeMap &map,

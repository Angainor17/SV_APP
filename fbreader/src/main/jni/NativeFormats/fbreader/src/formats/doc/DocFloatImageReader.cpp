...
DocFloatImageReader::DocFloatImageReader(unsigned int off, unsigned int len,
                                         shared_ptr<OleStream> tableStream,
                                         shared_ptr<OleStream> mainStream) :
...
ZLFileImage::Blocks DocFloatImageReader::getBlocksForShapeId(unsigned int shapeId) const {
    ...
    for (std::size_t i = 0; i < container.fopte.size(); ++i) {
        ...
        if (fopte.pId == 0x0104 &&
            !fopte.isComplex) { //0x0104 specifies the BLIP, see p.420 [MS-ODRAW]
...
unsigned int
DocFloatImageReader::readRecordHeader(RecordHeader &header, shared_ptr<OleStream> stream) {
...
unsigned int DocFloatImageReader::readDggContainer(OfficeArtContent &item, unsigned int length,
                                                   shared_ptr<OleStream> stream,
                                                   shared_ptr<OleStream> mainStream) {
...
unsigned int DocFloatImageReader::readBStoreContainer(OfficeArtContent &item, unsigned int length,
                                                      shared_ptr<OleStream> stream,
                                                      shared_ptr<OleStream> mainStream) {
...
unsigned int
DocFloatImageReader::skipRecord(const RecordHeader &header, shared_ptr<OleStream> stream) {
    stream->seek(header.length, false);
    return header.length;
}

unsigned int
DocFloatImageReader::readBStoreContainerFileBlock(Blip &blip, shared_ptr<OleStream> stream,
                                                  shared_ptr<OleStream> mainStream) {
    ...
    if (blip.storeEntry.offsetInDelay != (unsigned int) -1) {
        if (mainStream->seek(blip.storeEntry.offsetInDelay, true)) { //see p.70 [MS-ODRAW]
            ...
            ZLLogger::Instance().println("DocPlugin",
                                         "DocFloatImageReader: problems with seeking for offset");
            return count;
        }
    }
    RecordHeader header;
...
unsigned int DocFloatImageReader::readBlip(Blip &blip, const RecordHeader &header,
                                           shared_ptr<OleStream> stream) {
...
unsigned int DocFloatImageReader::readDgContainer(OfficeArtContent &item, unsigned int length,
                                                  shared_ptr<OleStream> stream) {
...
unsigned int DocFloatImageReader::readSpgrContainer(OfficeArtContent &item, unsigned int length,
                                                    shared_ptr<OleStream> stream) {
...
unsigned int DocFloatImageReader::readSpContainter(FSPContainer &item, unsigned int length,
                                                   shared_ptr<OleStream> stream) {
...
unsigned int
DocFloatImageReader::readArrayFOPTE(std::vector<FOPTE> &fopteArray, unsigned int length,
                                    shared_ptr<OleStream> stream) {

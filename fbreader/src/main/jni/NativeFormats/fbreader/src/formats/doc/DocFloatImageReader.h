...
public:
    DocFloatImageReader(unsigned int off, unsigned int len, shared_ptr<OleStream> tableStream,
...
private:
    ...
    static unsigned int
    readDggContainer(OfficeArtContent &item, unsigned int length, shared_ptr<OleStream> stream,
                     shared_ptr<OleStream> mainStream);

    static unsigned int
    readBStoreContainer(OfficeArtContent &item, unsigned int length, shared_ptr<OleStream> stream,
                        shared_ptr<OleStream> mainStream);

    static unsigned int readBStoreContainerFileBlock(Blip &blip, shared_ptr<OleStream> stream,
                                                     shared_ptr<OleStream> mainStream);

    static unsigned int
    readBlip(Blip &blip, const RecordHeader &header, shared_ptr<OleStream> stream);
    ...
    static unsigned int
    readArrayFOPTE(std::vector<FOPTE> &fopte, unsigned int length, shared_ptr<OleStream> stream);

    static unsigned int readFSP(FSP &fsp, shared_ptr<OleStream> stream);

    static unsigned int
    readSpContainter(FSPContainer &item, unsigned int length, shared_ptr<OleStream> stream);

    static unsigned int
    readSpgrContainer(OfficeArtContent &item, unsigned int length, shared_ptr<OleStream> stream);

    static unsigned int
    readDgContainer(OfficeArtContent &item, unsigned int length, shared_ptr<OleStream> stream);

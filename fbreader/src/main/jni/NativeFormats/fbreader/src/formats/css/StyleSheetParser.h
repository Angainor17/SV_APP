...
protected:
    StyleSheetMultiStyleParser(const std::string &pathPrefix, shared_ptr<FontMap> fontMap,
                               shared_ptr<EncryptionMap> encryptionMap);

protected:
    virtual void
    store(shared_ptr<CSSSelector> selector, const StyleSheetTable::AttributeMap &map) = 0;
...
public:
    StyleSheetTableParser(const std::string &pathPrexix, StyleSheetTable &styleTable,
                          shared_ptr<FontMap> fontMap, shared_ptr<EncryptionMap> encryptionMap);
...
inline StyleSheetParserWithCache::Entry::Entry(shared_ptr<CSSSelector> selector,
                                               const StyleSheetTable::AttributeMap &map) : Selector(
        selector), Map(map) {
}

...
public:
    OleMainStream(shared_ptr<OleStorage> storage, OleEntry oleEntry,
...
private: //readPieceTable helpers methods
    ...
    static void
    splitPieces(const Pieces &source, Pieces &dest1, Pieces &dest2, Piece::PieceType type1,
...
private: //formatting reader helpers methods
    ...
    static void getCharInfo(unsigned int chpxOffset, unsigned int styleId, const char *grpprlBuffer,
                            unsigned int bytes, CharInfo &charInfo);

    static void getStyleInfo(unsigned int papxOffset, const char *grpprlBuffer, unsigned int bytes,
                             Style &styleInfo);

    static void
    getSectionInfo(const char *grpprlBuffer, std::size_t bytes, SectionInfo &sectionInfo);

    static bool
    getInlineImageInfo(unsigned int chpxOffset, const char *grpprlBuffer, unsigned int bytes,
                       InlineImageInfo &pictureInfo);

    static Style getStyleFromStylesheet(unsigned int styleId, const StyleSheet &stylesheet);

    static int getStyleIndex(unsigned int styleId, const std::vector<bool> &isFilled,
                             const StyleSheet &stylesheet);

    static unsigned int
    getStyleIdByCharPos(unsigned int offset, const StyleInfoList &styleInfoList);

    static bool offsetToCharPos(unsigned int offset, unsigned int &charPos, const Pieces &pieces);

    static bool
    readToBuffer(std::string &result, unsigned int offset, std::size_t length, OleStream &stream);

...
OleStream::OleStream(shared_ptr<OleStorage> storage, OleEntry oleEntry,
...
std::size_t OleStream::read(char *buffer, std::size_t maxSize) {
    ...
    std::size_t sectorSize = (std::size_t) (myOleEntry.isBigBlock ? myStorage->getSectorSize()
                                                                  : myStorage->getShortSectorSize());
    ...
    for (std::size_t i = 0; i < toReadBlocks; ++i) {
        ...
        myBaseStream->seek(newFileOffset, true);
        readedBytes += myBaseStream->read(buffer + readedBytes,
                                          std::min(length - readedBytes, sectorSize));
    }
...
bool OleStream::seek(unsigned int offset, bool absoluteOffset) {
    ...
    unsigned int sectorSize = (myOleEntry.isBigBlock ? myStorage->getSectorSize()
                                                     : myStorage->getShortSectorSize());

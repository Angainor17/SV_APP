...
HtmlControlTagAction::HtmlControlTagAction(HtmlBookReader &reader, FBTextKind kind) : HtmlTagAction(
        reader), myKind(kind) {
}
...
HtmlHeaderTagAction::HtmlHeaderTagAction(HtmlBookReader &reader, FBTextKind kind) : HtmlTagAction(
        reader), myKind(kind) {
}
...
HtmlBreakTagAction::HtmlBreakTagAction(HtmlBookReader &reader, BreakType breakType) : HtmlTagAction(
        reader), myBreakType(breakType) {
}
...
HtmlListTagAction::HtmlListTagAction(HtmlBookReader &reader, int startIndex) : HtmlTagAction(
        reader), myStartIndex(startIndex) {
}
...
HtmlBookReader::HtmlBookReader(const std::string &baseDirectoryPath, BookModel &model,
                               const PlainTextFormat &format, const std::string &encoding)
        : HtmlReader(encoding), myBookReader(model), myBaseDirPath(baseDirectoryPath),
          myFormat(format), myBuildTableOfContent(true), myProcessPreTag(true) {
...
void
HtmlBookReader::preformattedCharacterDataHandler(const char *text, std::size_t len, bool convert) {
...
void HtmlBookReader::startDocumentHandler() {
    ...
    myDontBreakParagraph = false;
    for (std::map<std::string, shared_ptr<HtmlTagAction> >::const_iterator it = myActionMap.begin();
         it != myActionMap.end(); ++it) {
        it->second->reset();
    }
    myIsStarted = false;

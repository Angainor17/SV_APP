...
Тулбары должны скрываться при скролле списка под ними. Они должны быть видны только когда список
проскроллен в самое начало.
...
- `commonui/src/main/java/su/sv/commonui/ui/components/AppToolbar.kt` — добавлен `scrollBehavior` в
  `AppToolbarWithThemeToggle` и `AppToolbarSimple`
...
Поиск должен происходить после остановки ввода пользователя, только если под полем поиска не

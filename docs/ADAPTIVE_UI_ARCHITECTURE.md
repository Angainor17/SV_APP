...
Документ описывает архитектуру адаптивного UI в SV APP, который автоматически подстраивается под
размер экрана:
...
Не использовать ветвление внутри Composable (`if (isTablet) { ... } else { ... }`), а создавать *
*отдельные компоненты** для каждого форм-фактора.
